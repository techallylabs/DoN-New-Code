	...
	function help()
	{
		...
		if($screen->id == 'settings_page_' . $this->identifier)
		{
			$general_tab = '<p>' . esc_html__('Tips for the settings are located below select options.', 'solustrid-core') .
				...
				sprintf(esc_html__("%sTutorials and How Tos%s: There are several guides, tutorials, and how tos available on the author's website.", 'solustrid-core'),'<a title="' . esc_attr__('Go to the Breadcrumb NavXT tag archive.', 'solustrid-core') . '" href="https://mtekk.us/archives/tag/breadcrumb-navxt">', '</a>') . '</li><li>' .
				sprintf(esc_html__('%sOnline Documentation%s: Check out the documentation for more indepth technical information.', 'solustrid-core'), '<a title="' . esc_attr__('Go to the Breadcrumb NavXT online documentation', 'solustrid-core') . '" href="https://mtekk.us/code/breadcrumb-navxt/breadcrumb-navxt-doc/">', '</a>') . '</li><li>' .
				sprintf(esc_html__('%sReport a Bug%s: If you think you have found a bug, please include your WordPress version and details on how to reproduce the bug.', 'solustrid-core'),'<a title="' . esc_attr__('Go to the Breadcrumb NavXT support post for your version.', 'solustrid-core') . '" href="https://wordpress.org/support/plugin/breadcrumb-navxt/">', '</a>') . '</li></ul>' . 
				'<h5>' . esc_html__('Giving Back', 'solustrid-core') . '</h5><ul><li>' .
				sprintf(esc_html__('%sDonate%s: Love Breadcrumb NavXT and want to help development? Consider buying the author a beer.', 'solustrid-core'),'<a title="' . esc_attr__('Go to PayPal to give a donation to Breadcrumb NavXT.', 'solustrid-core') . '" href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=FD5XEU783BR8U&lc=US&item_name=Breadcrumb%20NavXT%20Donation&currency_code=USD&bn=PP%2dDonationsBF%3abtn_donateCC_LG%2egif%3aNonHosted">', '</a>') . '</li><li>' .
				sprintf(esc_html__('%sTranslate%s: Is your language not available? Visit the Breadcrumb NavXT translation project on WordPress.org to start translating.', 'solustrid-core'),'<a title="' . esc_attr__('Go to the Breadcrumb NavXT translation project.', 'solustrid-core') . '" href="https://translate.wordpress.org/projects/wp-plugins/breadcrumb-navxt">', '</a>') . '</li></ul>';
		...
		<form action="<?php echo $this->admin_url(); ?>" method="post" id="bcn_admin-options">
			...
			foreach($wp_post_types as $post_type)
			{
				...
				<table class="form-table adminkit-enset-top">
					...
					<tr valign="top">
						<th scope="row">
							<label for="<?php echo $optid;?>"><?php printf(esc_html__('%s Root Page', 'solustrid-core'), $post_type->labels->singular_name);?></label>
						</th>
						<td>
			...
			<fieldset id="miscellaneous" class="bcn_options">
				...
				<table class="form-table">
					...
					<tr valign="top">
						<th scope="row">
							<label for="<?php echo $optid;?>"><?php esc_html_e('Author Root Page', 'solustrid-core');?></label>
						</th>
						<td>
					...
					<tr valign="top">
						...
						<td>
							<label>
								...
								<?php printf(esc_html__('Limit the length of the breadcrumb title. (Deprecated, %suse CSS instead%s)', 'solustrid-core'), '<a title="' . esc_attr__('Go to the guide on trimming breadcrumb title lengths with CSS', 'solustrid-core') . '" href="https://mtekk.us/archives/guides/trimming-breadcrumb-title-lengths-with-css/">', '</a>');?>
							</label><br />
							<ul>
								<li>
