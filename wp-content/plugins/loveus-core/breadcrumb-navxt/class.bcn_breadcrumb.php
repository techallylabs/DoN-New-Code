...
class bcn_breadcrumb
{
	...
	{
		return sprintf('<span property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" title="%1$s" href="%%link%%" class="%%type%%"><span property="name">%%htitle%%</span></a><meta property="position" content="%%position%%"></span>', esc_attr__('Go to %title%.','wokiee-core'));
	}
	/**
	...
	public function assemble($linked, $position)
	{
		...
		{
			_doing_it_wrong(__CLASS__ . '::' . __FUNCTION__, __('bcn_breadcrumb::type must be an array', 'wokiee-core'), '6.0.2');	
		}
